...
class CCOMPortReaderThread(QtCore.QThread):
                    ...
                    while i < 60 and not self.close:
                        if self.port.inWaiting() > 0:
                            while self.port.inWaiting() > 0:
                                message.append(self.port.read())
                        QtCore.QThread.msleep(50)
                        i += 1

                    if message:
                        try:
                            self.read.emit(function(message))
    ...
    availableModes = {
        'oms': CCOMPortWorker.readOMSBarCode,
        'action': CCOMPortWorker.readActionBarCode,
        'other': lambda message: message
    }

    def __init__(self, port, mode='oms'):
        ...
        self.barCodeReader = self.CBarCodeWorker(self.port)
    ...
    def run(self):
        self.barCodeReader.readData(self.availableModes.get(self.mode, self.availableModes['other']))


def enableBarCodeReader(
        barCodethread,
        onBarCodeReaderMethod,
    ...
    if createNew or (barCodethread and barCodethread.isFinished()):
        disableBarCodeReader()
        barCodethread = CCOMPortReaderThread(port, mode)
        barCodethread.start()

    barCodethread.barCodeReader.read.connect(onBarCodeReaderMethod)
    barCodethread.barCodeReader.error.connect(onBarCodeReaderError)
    barCodethread.busyByEvent = busyByEvent


def disableBarCodeReader(barCodethread):
    if barCodethread:
        try:
            barCodethread.busyByEvent = False
            barCodethread.disable()
            barCodethread.exit()
        except:
            pass
